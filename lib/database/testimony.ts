...
import { testimonySources } from "@/lib/database/schema";
import { eq, sql } from "drizzle-orm";
import type { TestimonyEntry } from "@/lib/shared";
...
export async function getTestimoniesByIds(ids: string[]): Promise<any[]> {
  ...
  try {
    ...
    const results = await db
      ...
      .from(testimonySources)
      .where(
        sql`${testimonySources.id} IN (${sql.join(
          ids.map((id) => sql`${id}`),
          sql`, `
        )})`
      );
