      ...
      const hybridResults = await hybridSearch({
        ...
        embedFn:
          sourceType === "lexicon"
            ...
            : generateTestimonyEmbeddings,
        table:
          sourceType === "lexicon" ? lexiconEmbeddings : testimonyEmbeddings,
        embeddingColumn: sql`${
          sourceType === "lexicon"
            ? lexiconEmbeddings.embedding
            : testimonyEmbeddings.embedding
        }`,
        contentColumn: sql`${
          sourceType === "lexicon"
            ? lexiconEmbeddings.content
            : testimonyEmbeddings.content
        }`,
        joinTable: sourceType === "lexicon" ? lexiconSources : testimonySources,
        joinCondition:
          sourceType === "lexicon"
            ? eq(lexiconEmbeddings.resourceId, lexiconSources.id)
            : eq(testimonyEmbeddings.testimonyId, testimonySources.id),
        additionalColumns:
          sourceType === "lexicon"
            ? {
